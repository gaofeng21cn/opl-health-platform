...
机器可读边界由 `contracts/opl-health-platform-boundary.json` 记录。当前阶段只允许把医疗产品需求、能力包、审查策略、部署模型和 Cloud 能力引用映射落在本仓；运行时、工作空间执行、资源调度、模型网关、账单、证据存储、发布 currentness 和 owner receipt 仍归各自实现面。
